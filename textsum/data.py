...
import glob # unix shell风格路径名字匹配。可以用*,?（匹配一个）,[]进行多匹配，如果文件以'.'号开始，不会被默认自动匹配（即不能通过*进行匹配，必须通过显示标明.进行匹配
import random
import struct
import sys
...
class Vocab(object):
  '''这里直接解析vocab文件，如果说文件中每行没有两个element，省略，将word出现的行数作为它的id，
  vocab文件是一个有word和word的词频的文件（word occurences）是文件组成'''
...
def ExampleGen(data_path, num_epochs=None):
  ...
  while True:
    if num_epochs is not None and epoch >= num_epochs:
      break
    filelist = glob.glob(data_path) #返回匹配data_path的path names list（可能为空）
    ...
    for f in filelist:
      ...
      while True:
        ...
        str_len = struct.unpack('q', len_bytes)[0] #struct.unpack(fmt, buffer)('q'在python中代表8个字节的整数，C中是long long 类型）返回包含一个item的tuple
...
def GetWordIds(text, vocab, pad_len=None, pad_id=None):
  ...
  for w in text.split():
    ...
    if i >= 0:
      ...
      ids.append(vocab.WordToId(UNKNOWN_TOKEN)) # 这里直接上面那句可以执行吧，因为在WordToId中已经考虑了找不到当前word的情况
...
def Ids2Words(ids_list, vocab):
  ...
  return [vocab.IdToWord(i) for i in ids_list] # 这里不是会引发异常吗？这里如果找到不到word对应的Id的时候
...
# 这个函数没有使用
def GetExFeatureText(ex, key): # exmaple中的features是一个嵌套字典 features{feature{key:''value{bytes_list/float_list{value:}}}}
